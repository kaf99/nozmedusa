    ...
    describe("GET /admin/views/:entity/columns", () => {
      describe("orders entity", () => {
        ...
        it("should return all order columns including relationships", async () => {
          const response = await api.get(`/admin/views/orders/columns`, adminHeaders)
          ...
          const displayIdColumn = response.data.columns.find(c => c.id === "display_id")
          ...
          const salesChannelField = response.data.columns.find(c => c.id === "sales_channel")
          expect(salesChannelField).toBeUndefined()
          
          // Check that sales_channel.name is marked as default visible
          const salesChannelNameField = response.data.columns.find(c => c.id === "sales_channel.name")
          ...
          const salesChannelIdField = response.data.columns.find(c => c.id === "sales_channel.id")
          expect(salesChannelIdField).toBeDefined()
          expect(salesChannelIdField.default_visible).toBe(false)
          
          const salesChannelDescriptionField = response.data.columns.find(c => c.id === "sales_channel.description")
          ...
          const customerDisplayField = response.data.columns.find(c => c.id === "customer_display")
          ...
          const countryField = response.data.columns.find(c => c.id === "country")
          ...
          const customerField = response.data.columns.find(c => c.id === "customer")
          expect(customerField).toBeUndefined()
          
          const shippingAddressField = response.data.columns.find(c => c.id === "shipping_address")
      ...
      describe("products entity", () => {
        it("should return product columns", async () => {
          const response = await api.get(`/admin/views/products/columns`, adminHeaders)
          ...
          const titleColumn = response.data.columns.find(c => c.id === "title")
          ...
          const handleColumn = response.data.columns.find(c => c.id === "handle")
      ...
      describe("missing entity parameter", () => {
        it("should return 400 when entity is missing", async () => {
          const response = await api
            .get(`/admin/views//columns`, adminHeaders)
            .catch(e => e.response)
