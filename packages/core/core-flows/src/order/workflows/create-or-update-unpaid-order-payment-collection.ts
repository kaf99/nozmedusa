 ...
 */
export type CreateOrUpdateUnpaidOrderPaymentCollectionInput = {
  /**
...
export const createOrUpdateUnpaidOrderPaymentCollectionWorkflowId =
  "create-or-update-unpaid-order-payment-collection"
/**
 ...
 * - if the order has an unpaid (status is not paid or awaiting) payment collection, we updated the amount of the payment collection
 * - if the order doesn't have any payment collection, we create a new payment collection with the specified amount
 *
 * You can use this workflow within your customizations or your own custom workflows, allowing you to wrap custom logic around
 * creating or updating payment collections for an order.
 *
 * @example
 * const { result } = await createOrUpdateUnpaidOrderPaymentCollectionWorkflow(container)
 * .run({
 *   input: {
 *     order_id: "order_123",
 *     amount: 20
 ...
 */
export const createOrUpdateUnpaidOrderPaymentCollectionWorkflow =
  createWorkflow(
    createOrUpdateUnpaidOrderPaymentCollectionWorkflowId,
    (
      input: WorkflowData<{
        order_id: string
        amount?: number
