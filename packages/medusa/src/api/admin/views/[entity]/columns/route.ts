...
const getColumnCategory = (fieldName: string, dataType: string, semanticType?: string): HttpTypes.AdminColumn['category'] => {
...
export const GET = async (
  ...
  try {
    ...
    // Found joiner configs for schema introspection
    ...
    for (const config of joinerConfigs) {
      if (config.schema) {
        ...
        if (config.schema.includes(`type ${entityMapping.graphqlType} {`)) {
          hasEntityType = true
          // Found schema for entity type in joiner config
        }
      }
    }
    ...
    if (hasEntityType && schemaFragments.length > 0) {
      ...
      // Filtered direct fields
      
      // Add display_id if it's missing from Order schema (common field that might not be in GraphQL schema)
      if (entity === "orders" && !directFields.includes("display_id")) {
        directFields.unshift("display_id")
        // Added missing display_id field for orders
      }
      ...
      if (entityType) {
        ...
        Object.entries(fields).forEach(([fieldName, field]) => {
          // Apply field exclusion rules
          if (shouldExcludeField(fieldName, entityMapping.fieldFilters)) {
            // Filtering out manual relationship based on rules
            return
          }
          ...
          if (isSingleRelationship(field.type)) {
            ...
            // Manual single relationship found
          } else if (isArrayField(field.type)) {
            // Skipping array relationship
          }
        })
      }
      ...
      if (directFields.length > 0) {
        ...
        const directColumns = directFields.map((fieldName) => {
          ...
          const typeInfo = fieldDef
            ...
            : getTypeInfoFromGraphQLType(null, fieldName) // Still run through type detection for manually added fields
        ...
        if (finalRelations.size > 0) {
          for (const [relationName, relatedTypeName] of finalRelations) {
            ...
            const relatedFields = allRelatedFields.filter((fieldName) => {
              ...
              const isArray = isArrayField(field.type)
              if (isArray) {
                // Filtering out array field in related type
                return false
              }

              // Apply entity-specific field filters to related fields as well
              if (shouldExcludeField(fieldName, entityMapping.fieldFilters)) {
                // Filtering out field based on rules in related type
                return false
              }
            ...
            // Found non-array fields in related type
        ...
        // Generated columns from schema introspection
    ...
    // No schema found, falling back to hardcoded fields
  } catch (schemaError) {
...
export const POST = async (
  ...
  // const userId = req.auth_context.actor_id
  // Column visibility update requested
