...
export const SaveViewDialog: React.FC<SaveViewDialogProps> = ({
  ...
  const { createViewConfiguration, updateViewConfiguration } = useViewConfiguration()
  ...
  const onSubmit = async (data: SaveViewFormData) => {
    ...
    try {
      if (editingView) {
        ...
        const updated = await updateViewConfiguration(editingView.id, updateData)
        onSaved(updated)
      } else {
        // Create new view
        if (!currentColumns) {
        ...
        const created = await createViewConfiguration(createData)
        onSaved(created)
      }
    } catch (error) {
